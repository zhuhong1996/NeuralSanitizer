...
def define_model(path,x,mask_normalized,trigger):
...
def reverse(modeltrigger):
    ...
    init3 = np.random.uniform(127.5,127.5,size=[1,32,32,1])
    ...
    y_conv_trigger,y_trigger = define_model('./model/'+modeltrigger+'/model',x,mask_normalized,trigger)
    y_conv_clean,y_clean = define_model('./model/'+modelclean+'/model',x,mask_normalized,trigger)
    ...
    for i in range(10):
        ...
        for j in range(100):
            ...
            sess.run(train_step,feed_dict={x:batch_images, y_t:label_trigger, w1:w1_value,w2:w2_value,w3:w3_value,threshold:2})
            sess.run(clip_mask)
        ...
        trigger_image = trigger_image*std+mean
