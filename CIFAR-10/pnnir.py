...
def batch_norm2(x, scope, is_training=True):
...
def weight_variable2(init):
    shape = init.shape
    # initial = tf.truncated_normal(shape, stddev=0.1)
    init = np.random.uniform(-1e-1,1e-1,size=shape)
    initial = tf.constant(init,dtype=tf.float32,shape=shape)
    return tf.Variable(initial,name="weight")


def bias_variable2(init):
    shape = init.shape
...
def pnnir_1(modelname):
    ...
    with tf.name_scope('block7'):
        ...
        W_conv7_1 = weight_variable2(block7_w1)
        b_conv7_1 = bias_variable2(block7_b1)
        x_conv7_1 = conv2d(x_conv6_2, W_conv7_1, stride=2) + b_conv7_1
        x_conv7_1 = batch_norm2(x_conv7_1,"batch_norm_7_1")
        x_conv7_1 = tf.nn.relu(x_conv7_1)
        
        W_conv7_2 = weight_variable2(block7_w2)
        b_conv7_2 = bias_variable2(block7_b2)
        x_conv7_2 = conv2d(x_conv7_1, W_conv7_2) + b_conv7_2
        x_conv7_2 = batch_norm2(x_conv7_2,"batch_norm_7_2")
        
        W_conv7_3 = weight_variable2(block7_w3)
        b_conv7_3 = bias_variable2(block7_b3)
        x_conv7_3 = conv2d(x_conv6_2, W_conv7_3, stride=2) + b_conv7_3
        x_conv7_3 = batch_norm2(x_conv7_3,"batch_norm_7_3")
    ...
    train_step = tf.train.AdamOptimizer(1e-3).minimize(cross_entropy,var_list=[W_conv7_1,b_conv7_1,W_conv7_2,b_conv7_2,W_conv7_3,b_conv7_3])
    ...
    for i in range(10000):
        ...
        batch_images, batch_labels = get_next_clean_batch(50)
        train_step2.run(feed_dict={x: batch_images, y_: batch_labels})
        
        if i>0 and i%100==0:
            train_accuracy = 0
            for j in range(4):      
                batch_images, batch_labels = get_next_clean_batch(50)  
                train_accuracy += accuracy.eval(session=sess,feed_dict={x:batch_images, y_: batch_labels})
            train_accuracy/=4.0
...
def pnnir_2(modelname):
    ...
    with tf.name_scope('block8'):
        ...
        W_conv8_1 = weight_variable2(block8_w1)
        b_conv8_1 = bias_variable2(block8_b1)
        x_conv8_1 = conv2d(x_conv7_2, W_conv8_1) + b_conv8_1
        x_conv8_1 = batch_norm2(x_conv8_1,"batch_norm_8_1")
        x_conv8_1 = tf.nn.relu(x_conv8_1)
        
        W_conv8_2 = weight_variable2(block8_w2)
        b_conv8_2 = bias_variable2(block8_b2)
        x_conv8_2 = conv2d(x_conv8_1, W_conv8_2) + b_conv8_2
        x_conv8_2 = batch_norm2(x_conv8_2,"batch_norm_8_2")
        x_conv8_2 = x_conv7_2 + x_conv8_2
        x_conv8_2 = tf.nn.relu(x_conv8_2)
    ...
    cross_entropy = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits_v2(labels=y_, logits=y_conv))
    train_step = tf.train.AdamOptimizer(1e-3).minimize(cross_entropy,var_list=[W_conv8_1,b_conv8_1,W_conv8_2,b_conv8_2])
    train_step2 = tf.train.AdamOptimizer(1e-3).minimize(cross_entropy)
...
def pnnir_3(modelname):
    ...
    with tf.name_scope('fc'):
        W_fc = weight_variable2(fc_w)
        b_fc = bias_variable2(fc_b)
        y_conv = tf.matmul(pooling, W_fc) + b_fc

        
    cross_entropy = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits_v2(labels=y_, logits=y_conv))
    train_step = tf.train.AdamOptimizer(1e-3).minimize(cross_entropy,var_list=[W_fc,b_fc])
    train_step2 = tf.train.AdamOptimizer(1e-3).minimize(cross_entropy)
...
def pnnir_12(modelname):
    ...
    cross_entropy = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits_v2(labels=y_, logits=y_conv))
    train_step = tf.train.AdamOptimizer(1e-3).minimize(cross_entropy,var_list=[W_conv7_1,b_conv7_1,W_conv7_2,b_conv7_2,W_conv7_3,b_conv7_3,W_conv8_1,b_conv8_1,W_conv8_2,b_conv8_2])
    train_step2 = tf.train.AdamOptimizer(1e-3).minimize(cross_entropy)
...
def pnnir_123(modelname):
    ...
    cross_entropy = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits_v2(labels=y_, logits=y_conv))
    train_step = tf.train.AdamOptimizer(1e-3).minimize(cross_entropy,var_list=[W_conv7_1,b_conv7_1,W_conv7_2,b_conv7_2,W_conv7_3,b_conv7_3,W_conv8_1,b_conv8_1,W_conv8_2,b_conv8_2,W_fc,b_fc])
    train_step2 = tf.train.AdamOptimizer(1e-3).minimize(cross_entropy)
...
def pnnir_23(modelname):
    ...
    cross_entropy = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits_v2(labels=y_, logits=y_conv))
    train_step = tf.train.AdamOptimizer(1e-3).minimize(cross_entropy,var_list=[W_conv8_1,b_conv8_1,W_conv8_2,b_conv8_2,W_fc,b_fc])
    train_step2 = tf.train.AdamOptimizer(1e-3).minimize(cross_entropy)
...
def pnnir_13(modelname):
    ...
    cross_entropy = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits_v2(labels=y_, logits=y_conv))
    train_step = tf.train.AdamOptimizer(1e-3).minimize(cross_entropy,var_list=[W_conv7_1,b_conv7_1,W_conv7_2,b_conv7_2,W_conv7_3,b_conv7_3,W_fc,b_fc])
    train_step2 = tf.train.AdamOptimizer(1e-3).minimize(cross_entropy)
