...
def define_model_clean(path,x,keep_prob):
...
def reverse(modeltrigger):
    ...
    mask_ph = tf.placeholder(tf.float32, shape=[1, 28,28,1], name="mask_ph")
    trigger_ph = tf.placeholder(tf.float32, shape=[1, 28,28,1], name="trigger_ph") 
    threshold = tf.placeholder(tf.float32, name="threshold")
    
    initial = tf.truncated_normal([1,28,28,1], mean=0.5, stddev=0.1)    
    mask =  tf.Variable(initial,name="mask")
    
    mask_normalized = 0.5*(tf.tanh(10*(mask-0.5))+1)
    
    initial = tf.truncated_normal([1,28,28,1], mean=0, stddev=0.1)
    ...
    loss3 = w3*(tf.reduce_sum(tf.abs(mask_normalized[:,0:27,0:27,:]-mask_normalized[:,1:28,1:28,:]))\
                +tf.reduce_sum(tf.abs(mask_normalized[:,0:27,1:28,:]-mask_normalized[:,1:28,0:27,:]))\
                +tf.reduce_sum(tf.abs(mask_normalized[:,0:27,:,:]-mask_normalized[:,1:28,:,:]))\
                +tf.reduce_sum(tf.abs(mask_normalized[:,:,1:28,:]-mask_normalized[:,:,0:27,:])))
    ...
    for i in range(10):
        ...
        init3 = np.random.uniform(0.0,0.0,size=[1,28,28,1])
        init4 = np.random.uniform(-0.5,0.5,size=[1,28,28,1])
