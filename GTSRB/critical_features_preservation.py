...
trigger_image_indexs = np.zeros(shape=[43],dtype=np.int32)
def get_next_batch_all_trigger(batch_size,trigger,mask,source,target):
    ...
    for i in range(batch_size):
        ...
        trigger_image_indexs[label] = (image_index+1)%225
...
def preserve(modelname):
    ...
    one_hot = tf.sparse_to_dense(target_label, tf.stack([43]), 1.0,0.0)
